...
bool FEVTKExport::Save(FEModel& fem, const char* szfile)
{
	...
	if (m_bwriteAllStates)
	{
		...
		for (int is=0; is<ns; ++is) 
		{
			...
			if (WriteState(szname, fem, is) == false) return false;
		}
	...
	{
		return WriteState(szfile, fem, fem.currentTime());
	}
}        

bool FEVTKExport::WriteState(const char* szname, FEModel& fem, int nstate)
{
